...
- Fixed a crash when there is an error capturing microphone input. The
  error is now reported via `CallClientListener.onError(...)`.
...
- Added `StreamingLayout.RawTracksAudioOnly`, the `raw-tracks-audio-only`
  recording layout preset for audio-only raw-tracks recordings.

### Fixed

- Fixed an issue where participants would appear as "guest" in saved transcriptions.

- Fixed an issue where the `raw-tracks-audio-only` recording layout preset in a
  meeting token's `start_cloud_recording_opts` was ignored, causing raw-tracks
  recordings to include video.

- Fixed an issue where server-side `startRecording()` failures were reported
  as an opaque deserialization error instead of a proper recording error.
...
* If the screen share is terminated by the OS, the screen video input is switched off and
  `onInputsUpdated()` is triggered.
...
- Fixed an issue which caused occasional audio corruption when sending custom tracks

- Fixed a local reference leak when handling events
...
* Support for 16kB page size Android devices. Note that if you use the
  `daily-android-video-processor-plugin`, this should also be updated
  to `0.1.1`.

### Changed

* Android Target SDK version set to 35
